...
let swipesThisWeek = Array.from(table.rows).filter(x => !!Date.parse(x.cells[0].textContent) && Date.parse(x.cells[0].textContent) > new Date(Date.now() - new Date().getDay() * 86400000).setHours(0, 0, 0, 0));
let swipes = swipesThisWeek.length;

console.log(swipesThisWeek);
